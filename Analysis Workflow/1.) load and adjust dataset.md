...
Files '1-10_P4_barcodes.tsv.gz', '1-10_P4_features.tsv.gz', '1-10_P4_matrix.mtx.gz' were downloaded, renamed to 'barcodes.tsv.gz', 'features.tsv.gz', 'matrix.mtx.gz', respectively and transfered to a folder named "C:\scRNA-seq_Mouse_P4\filtered_bc_matrix\\" on a windows machine.
...
create folders on the system, e.g. on a windows machine "C:\user\[USERNAME]\Documents\R\R_Workspace\scRNAseqMouseHpc_P4_wt\\"

    # path <- "C:/Users/[USERNAME]/Documents/scRNAseqMouseHpc_P4_wt/" # works after adjusting 'USERNAME'

I prefer setting a working directory, this works with R with default installation on a windows machine:
...
filtered cell ranger output UMI-count matrix
    ...
    Scn2aP4.data <- Read10X(data.dir = "C:/scRNAseqMouseHpc_P4/filtered_bc_matrix/"]
...
uncomment to perform, destination path for saved file needs to be created and adjusted accordingly

    #saveRDS(Scn2a.P4, file = "~/R/scRNAseqMouseHpc/Scn2a_P4_meta.rds")
