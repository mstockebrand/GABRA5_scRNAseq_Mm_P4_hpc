...
#####  add columns with UMI-counts ('nCount_RNA') for each sample
    for (sample_no in levels(as.factor(meta$sample_no))) {
        ...
        metadata.summary[ , as.numeric(sample_no)] <- sample.summary
...
#####  add columns with percent mitochondrial gene counts for each sample, genotype, and total dataset
    ...
    for (genotype in levels(meta$genotype)) {
        ...
        metadata.summary[ , genotype] <- sample.summary
        colnames(metadata.summary)[ncol(metadata.summary)] <- paste0(genotype, "_pc.mito")
        }
            
    metadata.summary[ , "all_pc.mito"] <- c("all", unname(summary(meta$percent.mt)))
...
####  Violinplots of QC parameters
    Idents(MB_N5) <- "sample_no"
            VlnPlot(MB_N5, features = c("nCount_RNA","nFeature_RNA", "percent.mt", "percent.rb"), raster = FALSE, ncol = 2 )

<p align="center">
<img src="https://github.com/mstockebrand/Df16A_Dopaminergic_scRNA-seq/blob/main/Analysis workflow/Figures/QC_Figs/QC_1_preQC_violins.png" width="400" height="400"/>
</p>
...
<p align="center">
<img src="https://github.com/mstockebrand/Df16A_Dopaminergic_scRNA-seq/blob/main/Analysis workflow/Figures/QC_Figs/QC_2_nGenes_nUMI_preQC_scatter.png" width="800" height="400"/>
</p>


###  low quality cell filtering
####  extract modifiable QC statistics for modification
    Cell.QC.Stat <- MB_N5@meta.data


####  filtering cells by mitochondrial reads content [histogram-based threshold assignment]
    ##  set low and highthreshold
        max.mito.thr <- median(Cell.QC.Stat$percent.mt) + 40*mad(Cell.QC.Stat$percent.mt)   # max.mito.thr <- 0.1546177
...
<p align="center">
<img src="https://github.com/mstockebrand/Df16A_Dopaminergic_scRNA-seq/blob/main/Analysis workflow/Figures/QC_Figs/QC_3_mito_filt.png" width="400" height="400"/>
</p>
...
####  filtering cells by number of genes (nFeature_RNA) and transcripts (nCount_RNA) [histogram-based threshold assignment]
    ##  Set low and high thresholds on the number of detected genes
        min.Genes.thr <- median(log10(Cell.QC.Stat$nFeature_RNA)) - 2*mad(log10(Cell.QC.Stat$nFeature_RNA))
        max.Genes.thr <- median(log10(Cell.QC.Stat$nFeature_RNA)) + 2*mad(log10(Cell.QC.Stat$nFeature_RNA))    
        
    ##  Set high threshold on the number of transcripts
        min.nUMI.thr <- median(log10(Cell.QC.Stat$nCount_RNA)) - 1.4*mad(log10(Cell.QC.Stat$nCount_RNA))
        max.nUMI.thr <- median(log10(Cell.QC.Stat$nCount_RNA)) + 1.4*mad(log10(Cell.QC.Stat$nCount_RNA))
...
min.UMI.thr:    3.693;    max.UMI.thr:    5.087;    UMI per cell:    4,931 - 122,297  
min.Genes.thr:    3.395;    max.Genes.thr:    4.112;    Genes per cell:    2,484 -  12,945

<p align="center">
<img src="https://github.com/mstockebrand/Df16A_Dopaminergic_scRNA-seq/blob/main/Analysis workflow/Figures/QC_Figs/QC_4_feature_count_threschold_scatter.png" width="400" height="400"/>
</p>
