#  4.) Sample Integration and Normalization
...
##### Check heterogeneity captured by first and last created PCs
...
##### Determine 'dimensionality' of the dataset
    ## ElbowPlot of SD per PC
    ElbowPlot(Scn2a.P4, ndims = 50)
      
    ##  plot cumulative explained variance by PC
    explained_variance <- cumsum(Scn2a.P4@reductions$pca@stdev^2)/sum(Scn2a.P4@reductions$pca@stdev^2)
    
    par(mar=c(4,3,3,3))
    barplot((explained_variance), 
          main = "cumulative variance explained by PC", 
          xlab = "PC", 
          names.arg = c(1:50))
    abline(h = 0.95, col = "blue", lwd=2)
    abline(h = 0.98, col = "red", lwd=2)
    legend("right", c("95%", "98%"), fill = c("blue", "red"))
    
<p align="center">
<img src="https://github.com/mstockebrand/GABRA5_scRNAseq_Mm_P4_hpc/blob/main/Analysis Workflow/Figures/Integration_Figs/Int_4_PCA_elbow_cumVar.png" width="800" height="383"/>
</p>

##### select PCs for downstream analysis, cut-off = 98% explained variance
    ## PC explaining 95-98% Variance
    min(which(explained_variance > 0.95))
    min(which(explained_variance > 0.98))
    last_pc_id <- min(which(explained_variance > 0.98)) #42 

###  INTEGRATE DATASET
    Scn2a.P4 <- IntegrateLayers(object = Scn2a.P4, method = CCAIntegration, normalization.method = "SCT", verbose = F)

#####  re-join layers after integration
    Scn2a.P4[["RNA"]] <- JoinLayers(Scn2a.P4[["RNA"]])
